    ...
    <div class="card">
        ...
        <div id="report_copy_area" class="report-watermark">
            ...
            <details class="accordion" open>
                <summary>تحليل الشخصية والطالع والشمس والقمر — مجاني</summary>
            ...
            <details class="accordion" open>
                <summary>تحليل الكواكب واحدًا تلو الآخر — احترافي</summary>
                <div class="accordion-content">
                    {% for item in report.planetary_analysis %}<div class="item">{{ item }}</div>{% endfor %}
                </div>
            </details>
            <details class="accordion" open>
                <summary>تحليل الكويكبات والنقاط المهمة — احترافي</summary>
                <div class="accordion-content">
                    {% for item in report.asteroids_points_analysis %}<div class="item">{{ item }}</div>{% endfor %}
                </div>
            </details>
            <details class="accordion" open>
                <summary>تحليل البيوت الفلكية — احترافي</summary>
                <div class="accordion-content">
                    {% for item in report.houses_analysis %}<div class="item">{{ item }}</div>{% endfor %}
                </div>
            </details>
            <details class="accordion" open>
                <summary>الدستورية وحدود بطليموس والزوايا — احترافي</summary>
                <div class="accordion-content">
            ...
            <details class="accordion" open>
                <summary>نقاط المنتصف والجوانب العاطفية والمهنية والمالية — احترافي</summary>
                <div class="accordion-content">
            ...
            <div class="section">
                ...
                <div class="locked-grid">
                    <a class="locked-card" href="{{ premium_telegram_url }}" target="_blank" rel="noopener">
                        ...
                        <div class="locked-desc">قراءة تفصيلية للكواكب العشرة مع البرج، البيت، الحد، القوة الدستورية، والانعكاس النفسي.</div>
                        <div class="locked-cta">طلب فتح هذا القسم عبر تيليغرام</div>
                    </a>
                    <a class="locked-card" href="{{ premium_telegram_url }}" target="_blank" rel="noopener">
                        <span class="locked-badge">🔒 قسم احترافي</span>
                        <div class="locked-title">تحليل الكويكبات والنقاط المهمة</div>
                        <div class="locked-desc">كايرون، جونو، فيستا، بالاس، فولو، سيريس، ليليث، والعقد القمرية ضمن البرج والبيت والحد.</div>
                        <div class="locked-cta">طلب فتح هذا القسم عبر تيليغرام</div>
                    </a>
                    <a class="locked-card" href="{{ premium_telegram_url }}" target="_blank" rel="noopener">
                        <span class="locked-badge">🔒 قسم احترافي</span>
                    ...
                    <a class="locked-card" href="{{ premium_telegram_url }}" target="_blank" rel="noopener">
                        ...
                        <div class="locked-desc">تحليل الزوايا الصعبة والداعمة، الدستورية، نقاط الضغط، وطريقة تحويل التحدي إلى نضج.</div>
                        <div class="locked-cta">طلب فتح هذا القسم عبر تيليغرام</div>
                    </a>
                    <a class="locked-card" href="{{ premium_telegram_url }}" target="_blank" rel="noopener">
                        <span class="locked-badge">🔒 قسم احترافي</span>
...
def _marriage_period_text(item):
    ...
    if start and end:
        return f"{start.strftime('%Y-%m-%d')} إلى {end.strftime('%Y-%m-%d')}"
    return "غير محددة"
...
def build_marriage_free_summary(final_candidates, backup_candidates, all_results, start_age=None):
    """تقرير أولي مبسط: يعرض أقرب نافذة زمنية إلى بداية عمر الفحص فقط."""
    pool = list(all_results or [])
    chosen_item = None
    if start_age is not None:
        try:
            sa = int(start_age)
            future = [x for x in pool if int(x.get("age", -999)) >= sa]
            chosen_item = sorted(future, key=lambda x: int(x.get("age", 999)))[0] if future else None
        except Exception:
            chosen_item = None
    if chosen_item is None and pool:
        chosen_item = sorted(pool, key=lambda x: int(x.get("age", 999)))[0]
    if chosen_item is None:
        ranked = list(final_candidates or []) + list(backup_candidates or [])
        chosen_item = sorted(ranked, key=lambda x: int(x.get("age", 999)))[0] if ranked else None
    ...
    lines.append("أقرب فترة ظاهرة ضمن نطاق الفحص:")
    lines.append("")
    lines.append(f"العمر: {chosen_item.get('age')} سنة")
    lines.append(f"الفترة: {_marriage_period_text(chosen_item)}")
    ...
    lines.append("")
    lines.append("التقرير الاحترافي يعرض بقية الفترات مرتبة حسب العمر، مع ترجيح أيها أقرب للارتباط الفعلي.")
    return "\n".join(lines).strip()

def build_marriage_premium_simple_report(chart, ranked, all_results, name, gender, birth_info, city_name, start_age, end_age):
    """تقرير احترافي مبسط للقارئ: ترتيب زمني حسب العمر بلا مصطلحات فنية ثقيلة."""
    # نحسب الفلترة داخليًا فقط، لكن لا نذكرها للقارئ
    try:
    ...
    # نعرض الفترات ذات القوة المقبولة فقط، مرتبة حسب العمر لا حسب القوة
    visible = [x for x in items if _marriage_item_score(x) >= 55]
    if not visible:
        visible = list(items)[:5]
    visible = sorted(visible, key=lambda x: int(x.get("age", 999)))[:12]
    ...
    if visible:
        strongest_official = [x for x in visible if "زواج" in _marriage_event_text(x) or "خطوبة" in _marriage_event_text(x)]
        best = max(strongest_official or visible, key=lambda x: _marriage_item_score(x))
        lines.append(f"أبرز فترة تستحق الانتباه هي عمر {best.get('age')} سنة، بقوة {_marriage_item_score(best)} من 100، ودلالتها: {_marriage_event_text(best)}.")
    else:
        lines.append("لا تظهر فترة قوية جدًا داخل هذا النطاق، لكن توجد فترات يمكن مراقبتها بحذر.")
    lines.append("")
    lines.append("الفترات مرتبة زمنيًا حسب العمر:")
    lines.append("------------------------------")
    ...
    for i, item in enumerate(visible, start=1):
        ...
        lines.append("")
        lines.append(f"{i}. عمر {item.get('age')} سنة")
        lines.append(f"الفترة: {_marriage_period_text(item)}")
        lines.append(f"القوة: {score} من 100 / {marriage_strength_label(score)}")
    ...
    if visible:
        official = [x for x in visible if "زواج" in _marriage_event_text(x) or "خطوبة" in _marriage_event_text(x)]
        if official:
            top = max(official, key=lambda x: _marriage_item_score(x))
            lines.append(f"إذا كان السؤال عن زواج أو خطوبة رسمية، فأقوى فترة ظاهرة هي عمر {top.get('age')} سنة.")
        else:
            top = max(visible, key=lambda x: _marriage_item_score(x))
            lines.append(f"أقوى فترة عاطفية عامة هي عمر {top.get('age')} سنة، لكنها تحتاج ظروفًا واقعية حتى تتحول إلى ارتباط رسمي.")
    lines.append("")
    lines.append("ملاحظة مهمة:")
    lines.append("هذه قراءة فلكية رمزية تساعد على ترتيب الفترات ومراقبة الفرص، ولا تعني وعدًا قطعيًا بحدوث زواج دون توفر ظروف واقعية واختيار شخصي مناسب.")
    return "\n".join(lines).strip()
...
function showPremiumProgress(formId){const ov=document.getElementById('loadingOverlay');const bar=document.getElementById('marriageProgressBar');const pct=document.getElementById('marriageProgressPercent');const txt=document.getElementById('marriageProgressText');if(!ov)return true;ov.style.display='flex';function set(p,t){bar.style.width=p+'%';pct.innerText=p+'%';txt.innerText=t}set(8,'فحص كود الفتح...');setTimeout(function(){set(35,'إعادة حساب التقرير الاحترافي...')},500);setTimeout(function(){set(65,'استخراج العودة الشمسية والبروفكشن والفريدار...')},1200);setTimeout(function(){set(88,'كتابة التقرير الاحترافي الكامل...')},1900);setTimeout(function(){document.getElementById(formId).submit()},650);return false;}
        ...
        try:
            ...
            if str(request.form.get("action", "")).strip() == "premium":
                ...
                if not ok:
                    ...
                    premium_report = _REL_NS["build_professional_report"](c1, c2, analysis)
...
<div class="card"><h2>المحاور الأبرز</h2>{% for row in report.active_rows %}<div class="health-card"><h3>{{ row.system }} <span class="level">{{ row.percent }}% - {{ row.level }}</span></h3><div class="bar"><div class="fill" style="width:{{ row.percent }}%"></div></div><p>{{ row.meaning }}</p><p><strong>النصيحة:</strong> {{ row.advice }}</p>{% if row.reasons %}<details><summary>الأسباب الفلكية المختصرة</summary><ul>{% for r in row.reasons %}<li>{{ r }}</li>{% endfor %}</ul></details>{% endif %}</div>{% endfor %}</div>
<div class="card"><h2>المحاور الهادئة نسبيًا</h2><p>{{ report.calm_text }}</p></div>
<div class="card"><h2>مواقع الكواكب المستخدمة</h2><table class="small-table"><thead><tr><th>الكوكب</th><th>الموقع</th><th>البيت</th><th>الحركة</th></tr></thead><tbody>{% for p in report.planet_rows %}<tr><td>{{ p.name }}</td><td>{{ p.pos }}</td><td>{{ p.house }}</td><td>{{ p.motion }}</td></tr>{% endfor %}</tbody></table></div>
{% endif %}
<div class="footer">جميع الحقوق محفوظة للمطور astrologer.ab@</div>
</div></body></html>
"""
