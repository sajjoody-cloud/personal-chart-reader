...
def generate_service_order_id(prefix: str = "AGM") -> str:
    """رقم طلب بسيط لخدمات تيليغرام والاشتراك الشهري."""
    now = datetime.now()
...
def monthly_subscription_message(order_id: str, name: str = "") -> str:
    clean_name = (name or "").strip() or "غير مذكور"
    return (
        "السلام عليكم، أرغب بالاشتراك في خدمة التوقعات الشهرية الشخصية عبر ASTROGATE.\n"
...
def premium_section_message(order_id: str, name: str = "") -> str:
    """رسالة طلب فتح قسم احترافي مقفل؛ مستقلة عن الاشتراك الشهري."""
    clean_name = (name or "").strip() or "غير مذكور"
    return (
        ...
        "ملاحظة: هذا طلب تحليل مثمّن لقسم محدد وليس اشتراكًا شهريًا."
    )
...
<div class="container">
    <h1>{{ title }}</h1>
    <div class="platform-nav"><a href="/">الرئيسية</a><a href="/profile">بياناتي الفلكية</a><a href="/natal">قراءة الخريطة</a><a href="/monthly-subscription">الاشتراك الشهري</a></div>
    ...
    <div class="card">
        ...
        <div id="report_copy_area" class="report-watermark">
            ...
            <div class="pro-service-card">
                <h3>متابعة شهرية مخصصة حسب خريطتك</h3>
                <p>انتهى الجزء المجاني من التقرير. يمكنك اختيار أحد مسارين: طلب فتح قسم احترافي محدد من الأقسام المقفلة، أو الاشتراك في المتابعة الشهرية الشخصية التي تشمل تقريرًا شهريًا خاصًا بالتوقعات والفترات المهمة.</p>
                <ul>
                    <li>تقرير شهري شخصي غير عام.</li>
                    <li>تحليل التوقعات حسب خريطة الميلاد وليس حسب البرج فقط.</li>
                    <li>المتابعة الشهرية خاصة بالتوقعات والفترات المهمة خلال الشهر.</li>
                    <li>متابعة عبر تيليغرام بدون نشر أرقام بطاقات أو بيانات دفع.</li>
                </ul>
                <p>رقم الطلب: <span class="order-code">{{ monthly_order_id }}</span></p>
                <a class="telegram-btn" href="{{ monthly_telegram_url }}" target="_blank" rel="noopener">طلب الاشتراك الشهري عبر تيليغرام</a>
            </div>
...
<div class="page">
    <section class="brand-card">
        ...
        <div class="tool-list">
            ...
            <a class="tool-item" href="/monthly-subscription">
                <span class="tool-icon">☾</span>
                <span class="tool-text">الاشتراك الشهري الشخصي</span>
                <span class="tool-badge open">متاح</span>
            </a>
        ...
        <div class="service-highlight">
            <h3>الاشتراك الشهري الشخصي</h3>
            <p>متابعة شهرية خاصة حسب خريطتك الفلكية، تركّز على أهم فرص الشهر وتحدياته في العمل، المال، العلاقات، الصحة والطاقة النفسية.</p>
            <div class="service-pills">
                <span>تقرير شهري مخصص</span>
                <span>تواريخ مهمة</span>
                <span>فرص وتحذيرات</span>
                <span>تواصل عبر تيليغرام</span>
            </div>
            <p>رقم الطلب الحالي: <span class="order-code">{{ monthly_order_id }}</span></p>
            <div class="service-actions">
                <a class="service-btn primary" href="{{ premium_telegram_url }}" target="_blank" rel="noopener">طلب الاشتراك عبر تيليغرام</a>
                <a class="service-btn secondary" href="/monthly-subscription">تفاصيل الاشتراك</a>
            </div>
        </div>
    </section>
    ...
    <div class="footer">
        ...
        <span> | </span>
        <a href="/monthly-subscription">الاشتراك الشهري</a>
        <span> | </span>
...
MONTHLY_SUBSCRIPTION_HTML = r"""
<!DOCTYPE html>
<html lang="ar" dir="rtl">
<head>
    <meta charset="UTF-8">
    <title>الاشتراك الشهري الشخصي - AstroGate</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <style>
        body { margin:0; font-family:Tahoma,Arial,sans-serif; background:linear-gradient(180deg,#160d22 0%,#241432 48%,#170d21 100%); color:#2d2926; line-height:1.9; }
        .page { max-width:850px; margin:0 auto; padding:14px 14px 28px; }
        .card { background:#f7f1e4; border:1px solid rgba(197,172,125,.72); border-radius:24px; box-shadow:0 18px 35px rgba(0,0,0,.22); padding:20px 18px; margin-bottom:16px; }
        .top { text-align:center; }
        .logo { width:96px; height:96px; border-radius:50%; border:2px solid #b99a5b; object-fit:cover; background:#050505; }
        h1 { margin:10px 0 4px; font-size:30px; color:#17120e; }
        h2 { font-size:22px; color:#33281f; margin:18px 0 8px; border-right:5px solid #9f7d4f; padding-right:10px; }
        p { font-size:17px; margin:8px 0; color:#332c25; }
        .grid { display:grid; grid-template-columns:repeat(2, minmax(0,1fr)); gap:12px; }
        .box { background:#fffdf8; border:1px solid #e2d6c5; border-radius:18px; padding:14px; }
        .box strong { color:#4b3828; }
        .order { direction:ltr; display:inline-block; font-weight:900; background:#fffdf8; border:1px dashed #9f7d4f; border-radius:10px; padding:2px 8px; color:#4b3828; }
        .btns { display:flex; gap:10px; flex-wrap:wrap; margin-top:18px; }
        .btn { flex:1; min-width:170px; text-align:center; text-decoration:none; border-radius:14px; padding:13px 14px; font-weight:900; }
...
<div class="page">
    <section class="card top">
        ...
        <h1>الاشتراك الشهري الشخصي</h1>
        <p>خدمة متابعة شهرية خاصة حسب خريطة الميلاد، وليست توقعات عامة لكل الأبراج.</p>
        <p>رقم الطلب: <span class="order">{{ order_id }}</span></p>
    </section>

    <section class="card">
        <h2>ماذا يتضمن الاشتراك؟</h2>
        <div class="grid">
            <div class="box"><strong>فرص الشهر</strong><br>أبرز الفترات التي تحمل دعمًا مهنيًا أو ماليًا أو عاطفيًا حسب خريطة الشخص.</div>
            <div class="box"><strong>فترات الحذر</strong><br>الأيام التي تحتاج هدوءًا أو تجنب قرارات سريعة أو ضغط نفسي زائد.</div>
            <div class="box"><strong>التواريخ المهمة</strong><br>تحديد النوافذ الزمنية الأقوى خلال الشهر بدل الكلام العام.</div>
            <div class="box"><strong>قراءة شخصية</strong><br>تعتمد على الطالع، الكواكب، البيوت، الزوايا، ونقاط التفعيل الخاصة بصاحب الطلب.</div>
        </div>
        ...
        <div class="btns">
            ...
            <a class="btn secondary" href="/">العودة للرئيسية</a>
        </div>
    </section>
...
<div class="page">
    <section class="card">
        ...
        <p>تبدأ المنصة حاليًا بأدوات قراءة الخريطة الشخصية والمؤشرات الصحية الفلكية، ثم ستتوسع تدريجيًا لتشمل التوقعات الشخصية، نقاط المنتصف، توافق العلاقات، توقيت الزواج، تصحيح الطالع، والمقالات الفلكية.</p>
...
def home():
    ...
    monthly_order_id = generate_service_order_id("AGM")
    saved_profile = session.get("astro_profile", {}) if isinstance(session.get("astro_profile", {}), dict) else {}
    monthly_telegram_url = telegram_contact_url(monthly_subscription_message(monthly_order_id, str(saved_profile.get("name", ""))))
...
def natal():
    ...
    city_suggestions = city_suggestions_for_country(form.get("country_code", ""))
    monthly_order_id = generate_service_order_id("AGM")
    monthly_telegram_url = telegram_contact_url(monthly_subscription_message(monthly_order_id, form.get("name", "")))
    premium_telegram_url = telegram_contact_url(premium_section_message(monthly_order_id, form.get("name", "")))
...
<div class="page"><div class="hero"><h1>توافق العلاقات</h1><p>تقرير أولي مجاني يوضح نسبة التوافق العامة، مع إمكانية فتح التقرير الاحترافي.</p></div><div class="nav"><a href="/">الرئيسية</a><a href="/profile">بياناتي الفلكية</a><a href="/natal">قراءة الخريطة</a><a href="/forecast">التوقعات الشخصية</a><a href="/marriage">الزواج والخطوبة</a><a href="/monthly-subscription">الاشتراك الشهري</a></div>
